...
PANEL = os.path.join(CARPETA, "panel_paises.csv")


def construir_panel_paises(cfg_panel):
    """
    Panel país-año para H2-países y H5-países:
    ansiedad y depresión por país (archivo IHME ya limpio, con ISO3)
    + usuarios de internet y población por país (Banco Mundial).
    """
    ihme = pd.read_csv(cfg_panel["ihme_archivo"])
    faltan = {"iso3", "anio", "A", "Dp"} - set(ihme.columns)
    if faltan:
        raise ValueError(f"archivo IHME por países sin columnas {sorted(faltan)}")
    paises = _paises_reales()
    con = _por_pais(cfg_panel.get("codigo_conexion", "IT.NET.USER.ZS"), paises)
    pob = _por_pais("SP.POP.TOTL", paises)
    con = con.rename(columns={"pais": "iso3", "valor": "conexion"})
    pob = pob.rename(columns={"pais": "iso3", "valor": "pob"})
    panel = (ihme[["iso3", "anio", "A", "Dp"]]
             .merge(con, on=["iso3", "anio"], how="left")
             .merge(pob, on=["iso3", "anio"], how="left")
             .sort_values(["iso3", "anio"]))
    panel.to_csv(PANEL, index=False)
    completos = panel.dropna(subset=["A", "Dp", "conexion"])
    return {"ok": True, "paises": int(completos["iso3"].nunique()),
            "filas_completas": int(len(completos)),
            "anios": [int(completos["anio"].min()), int(completos["anio"].max())]}
...
def main():
    ...
    if "panel_paises" in cfg:
        try:
            estado["panel_paises"] = construir_panel_paises(cfg["panel_paises"])
            estado["panel_paises"]["nombre"] = "Panel por países (IHME + Banco Mundial)"
            print("panel_paises OK")
        except Exception as e:
            nota = ("se conserva el panel de la corrida anterior"
                    if os.path.exists(PANEL) else "sin panel")
            estado["panel_paises"] = {"nombre": "Panel por países (IHME + Banco Mundial)",
                                      "ok": False, "error": str(e)[:200], "nota": nota}
            print(f"panel_paises FALLÓ ({nota})")
