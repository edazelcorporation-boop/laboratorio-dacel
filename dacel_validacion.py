...
def prueba_h3_v8_estructural(df,reporte,carpeta,etiqueta):
    ...
    """
    reporte.append("## H3 — Daçel estructural v8 (formulación principal)\n")
    reporte.append(
    ...
    for nombre,P in familias.items():
        ...
        linea=(
            f"- {nombre}: {nc}/{n} observaciones ({100*nc/n:.1f}%) muestran "
    ...
    if total_obs:
        reporte.append(
            f"- Cobertura conjunta descriptiva: {total_cambio}/{total_obs} "
            f"({100*prop:.1f}%) observaciones comparables presentan cambio estructural "
            "posterior/contemporáneo medible en el paso anual."
        )
        if supera_h3:
            reporte.append(
                f"**Veredicto H3-v8: SUPERA LA PRUEBA.** "
                f"Se observó transformación estructural medible en "
                f"{total_cambio}/{total_obs} casos comparables ({100*prop:.1f}%)."
            )
        else:
            reporte.append(
                f"**Veredicto H3-v8: NO SUPERA LA PRUEBA.** "
                f"Se observó transformación estructural medible en "
                f"{total_cambio}/{total_obs} casos comparables ({100*prop:.1f}%)."
            )
    else:
        reporte.append("**Veredicto H3-v8: SIN DATOS SUFICIENTES.**")
    reporte.append(
    ...
    reporte.append(
        "- Las antiguas H3-v4/v5/v6 quedan como auditorías históricas de formulaciones "
        "más estrechas (gradiente/correlación temporal) y no constituyen la H3 principal."
    )

    paquete={"version":"H3-v8","tipo":"compatibilidad estructural",
...
def ejecutar(df, etiqueta, carpeta, semilla=42):
    ...
    if h3v8.get("observaciones_totales",0):
        estado_h3 = "SUPERA LA PRUEBA" if h3v8.get("supera_prueba") else "NO SUPERA LA PRUEBA"
        reporte.append(
            f"- H3 vigente (Daçel estructural v8): **{estado_h3}** — "
            f"transformación observada {100*h3v8['proporcion_global']:.1f}% "
            f"({h3v8['transformaciones_observadas']}/{h3v8['observaciones_totales']})"
        )
    else:
        reporte.append("- H3 vigente (Daçel estructural v8): **SIN DATOS SUFICIENTES**")
    reporte.append("\nH3-v8 cuantifica la transformación observada tal como la define Daçel; "
                   "las predicciones derivadas se evaluarán por separado conforme lleguen nuevos datos.\n")
