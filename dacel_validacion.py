...
def prueba_h3_v8_estructural(df,reporte,carpeta,etiqueta):
    ...
    direccion=_dacel_direccion_estado(df)
    ...
    for nombre,P in familias.items():
        ...
        res={
            "estado":"COMPATIBLE" if nc==n else "EXCEPCIONES OBSERVADAS",
            ...
            "direccion":{"expansion_neta":exp,"contraccion_neta":con,
                         "cambio_neto_casi_nulo":neut,"sin_direccion":sindir},
        ...
        reporte.append(
            ...
            f"dirección neta: expansión {exp}, contracción {con}, casi nula {neut}"
            + (f", sin dirección {sindir}" if sindir else "") + "."
        )
...
def ejecutar(df, etiqueta, carpeta, semilla=42):
    ...
    veredictos = {
        ...
        "H3-v4 histórica (auditoría, formulación retirada)": prueba_h3(df, reporte, carpeta, etiqueta, rng_h3),
        "H3-v5 histórica (auditoría, formulación retirada)": prueba_h3_v5_historica(df, reporte, carpeta, etiqueta),
    ...
    if h3v8.get("observaciones_totales",0):
        reporte.append(
            f"- H3 vigente (Daçel estructural v8): **COMPATIBILIDAD OBSERVACIONAL "
            f"{100*h3v8['proporcion_global']:.1f}%** "
