...
VEREDICTO_CUMPLE = "CUMPLE EL POSTULADO BASE"
VEREDICTO_NOCUMPLE = "NO CUMPLE EL POSTULADO BASE"
ALFA = 0.05
...
def prueba_h3_v5_historica(df, reporte, carpeta, etiqueta, ventana=5):
    ...
    """
    reporte.append("### H3-v5 histórica — formulación inferencial retirada\n")
    reporte.append(
...
def prueba_h3v6_biologica_robustez(df,reporte,carpeta):
    """
    Controles de robustez POSTERIORES al resultado primario H3-v6.
    No sustituyen, recalibran ni cambian el veredicto primario h=3.
    Se prueban decisiones alternativas predefinidas y estabilidad temporal.
    """
    reporte.append("### H3-v6 Biológica — Robustez (secundaria, no cambia el resultado primario)")
    anual,meta=_who_anual_desde_archivos()
    if anual.empty or "cambio_tasa" not in anual:
        reporte.append("- WHO auditado: SIN DATOS para robustez.")
        return {"estado":"SIN DATOS","meta":meta}

    controles={"nota":"Análisis secundario. El primario permanece h=3, cobertura mínima 3 países."}

    # A) Sensibilidad al horizonte de respuesta. h=3 sigue siendo el único primario.
    horizontes={}
    for h in (1,3,5):
        ...
        horizontes[str(h)]=res
        if res.get("estado")=="SIN DATOS":
            reporte.append(f"- Horizonte h={h}: SIN DATOS (n={res.get('n',0)}).")
        else:
            reporte.append(
                f"- Horizonte h={h}: n={res['n']}; rho={res['rho']:+.3f}; "
                f"p circular={res['p_circular']:.4f}; {res['estado']}."
                + (" **PRIMARIO**" if h==3 else " secundario")
            )
    controles["horizontes"]=horizontes

    # B) Sensibilidad a cobertura mínima anual de países.
    coberturas={}
    R3=_h3v6_respuesta_global(df,h=3)
    for minimo in (3,5,10,20):
        s=anual["cambio_tasa"].where(anual["paises_con_cambio"]>=minimo)
        P=_percentil_exante_min(s,min_hist=8).dropna()
        res=_h3v6_evaluar_familia(P,R3)
        coberturas[str(minimo)]=res
        if res.get("estado")=="SIN DATOS":
            reporte.append(f"- Cobertura ≥{minimo} países: SIN DATOS (n={res.get('n',0)}).")
        else:
            reporte.append(
                f"- Cobertura ≥{minimo} países: n={res['n']}; rho={res['rho']:+.3f}; "
                f"p circular={res['p_circular']:.4f}; {res['estado']}."
            )
    controles["cobertura_minima"]=coberturas

    # C) Estabilidad temporal: mitades cronológicas de la intersección primaria.
    s=anual["cambio_tasa"].where(anual["paises_con_cambio"]>=3)
    P=_percentil_exante_min(s,min_hist=8).dropna()
    idx=P.index.intersection(R3.dropna().index).sort_values()
    temporal={}
    if len(idx)>=20:
        corte=int(idx[len(idx)//2])
        for nombre,mask in (
            ("temprano", idx[idx < corte]),
            ("tardio", idx[idx >= corte]),
        ):
            if len(mask)>=10:
                res=_h3v6_evaluar_familia(P.loc[mask],R3.loc[mask],min_n=10)
            else:
                res={"estado":"SIN DATOS","n":int(len(mask))}
            temporal[nombre]=res
            if res.get("estado")=="SIN DATOS":
                reporte.append(f"- Periodo {nombre}: SIN DATOS (n={res.get('n',0)}).")
            else:
                reporte.append(
                    f"- Periodo {nombre}: {int(mask.min())}–{int(mask.max())}; n={res['n']}; "
                    f"rho={res['rho']:+.3f}; p circular={res['p_circular']:.4f}; {res['estado']}."
                )
        controles["corte_temporal"]=corte
    controles["estabilidad_temporal"]=temporal

    # D) Sensibilidad a años próximos a grandes cambios de revisión ICD.
    # Control conservador: excluir ±1 año alrededor de 1968, 1979 y 1999.
    transiciones=(1968,1979,1999)
    excluir=set()
    for y in transiciones:
        excluir.update((y-1,y,y+1))
    Psin=P.loc[~P.index.isin(excluir)]
    Rsin=R3.loc[R3.index.intersection(Psin.index)]
    res_icd=_h3v6_evaluar_familia(Psin,Rsin,min_n=10)
    controles["sin_entorno_transiciones_icd"]={
        "anios_centrales":list(transiciones),
        "ventana_excluida":"+/-1 año",
        "resultado":res_icd
    }
    if res_icd.get("estado")=="SIN DATOS":
        reporte.append(f"- Sin ±1 año alrededor de cambios ICD: SIN DATOS (n={res_icd.get('n',0)}).")
    else:
        reporte.append(
            f"- Sin ±1 año alrededor de 1968/1979/1999: n={res_icd['n']}; "
            f"rho={res_icd['rho']:+.3f}; p circular={res_icd['p_circular']:.4f}; "
            f"{res_icd['estado']}."
        )

    reporte.append(
        "- Interpretación: estos controles son de robustez y no pueden convertir un resultado "
        "primario negativo en positivo ni borrar el resultado primario registrado."
    )
    with open(os.path.join(carpeta,"H3_v6_biologica_robustez.json"),"w",encoding="utf-8") as f:
        json.dump(controles,f,ensure_ascii=False,indent=2)
    return controles



def _dacel_direccion_estado(df, cols=None):
    """
    Dirección neta anual del estado observable. Se mantiene SEPARADA de la
    magnitud de transformación: una contracción sigue contando como cambio.
    """
    if cols is None:
        cols=["E","I","X","K","K_patentes","conexion"]
    cols=[c for c in cols if c in df.columns]
    if not cols:
        return pd.Series(dtype=float)
    z=df.set_index("anio")[cols].apply(pd.to_numeric,errors="coerce").sort_index()
    cambios=[]
    for c in cols:
        s=z[c]
        # Cambio relativo simétrico; admite ceros y evita imponer "crecimiento".
        den=(s.abs()+s.shift(1).abs()).replace(0,np.nan)
        cambios.append((2*(s-s.shift(1))/den).rename(c))
    g=pd.concat(cambios,axis=1)
    return g.median(axis=1,skipna=True).dropna()


def _familias_perturbacion():
    """Series ex-ante de perturbación por familia (compartidas por H3-v8 y H3-v9)."""
    familias={}; auditoria={}
    try:
        pg,mg=_serie_conflicto_ucdp()
        pb,mb=_serie_muertes_batalla()
        P=_combinar_canales_exante({"conflicto":pg,"muertes":pb},min_canales=1) if len(pg) and len(pb) else (pg if len(pg) else pb)
        familias["conflicto"]=P; auditoria["conflicto"]={"UCDP":mg,"battle_deaths":mb}
    except Exception as e:
        familias["conflicto"]=pd.Series(dtype=float); auditoria["conflicto"]={"estado":"ERROR","error":str(e)}
    try:
        P,m=_serie_sismos_usgs(); familias["geofísica"]=P; auditoria["geofísica"]=m
    except Exception as e:
        familias["geofísica"]=pd.Series(dtype=float); auditoria["geofísica"]={"estado":"ERROR","error":str(e)}
    try:
        wb,m=_series_wdi_multicausal()
        for k in ("económica","ecológica","tecnológica"):
            familias[k]=wb.get(k,pd.Series(dtype=float)); auditoria[k]=m
    except Exception as e:
        for k in ("económica","ecológica","tecnológica"):
            familias[k]=pd.Series(dtype=float); auditoria[k]={"estado":"ERROR","error":str(e)}
    try:
        P,m=_serie_biologica_who(); familias["biológica"]=P; auditoria["biológica"]=m
    except Exception as e:
        familias["biológica"]=pd.Series(dtype=float); auditoria["biológica"]={"estado":"ERROR","error":str(e)}

    return familias, auditoria


def prueba_h3_v8_estructural(df,reporte,carpeta,etiqueta):
    """
    H3-Daçel v8 — formulación estructural principal.

    Postulado operativo:
        perturbación observada -> transición/reorganización observable.

    No exige:
      * crecimiento positivo,
      * rho positivo,
      * p<0.05,
      * horizonte fijo de tres años.

    La perturbación P y la transformación T se construyen con canales distintos.
    La prueba informa COMPATIBILIDAD EMPÍRICA del postulado en los casos con
    datos antes/después; no convierte la definición en una prueba causal.
    """
    reporte.append("## H3 — Postulado base de reestructuración (verificación con datos)\n")
    reporte.append("Postulado Daçel: todo sistema perturbado se reestructura y se transforma, para bien "
                   "o para mal. Aquí se verifica con datos reales si, en los años de perturbación alta de "
                   "cada familia, el estado del sistema mundial cambió de forma medible.\n")
    reporte.append("Estatus: es el principio base de la teoría, no una hipótesis que compita contra un "
                   "rival. Su calificación es la cobertura observada (escala 0–100%: porcentaje de años "
                   "de perturbación alta con transformación medible). La prueba de intensidad, que sí "
                   "puede fallar, es H3-v9.\n")
    reporte.append(
        "Postulado operativo: `P → ΔS`. Una perturbación cuenta como exposición; "
        "la evidencia observacional es un cambio independiente del estado del sistema. "
        "Expansión, contracción, deterioro, sustitución, adaptación, recuperación o "
        "colapso cuentan como transformación. El signo normativo no decide H3.\n"
    )
    reporte.append(
        "Medición principal: `T(t→t+1)=D[x(t),x(t+1)]`, con D como distancia "
        "multivariable del estado observable. Se usa un paso anual para no imponer "
        "el antiguo cuello de botella h=3. La dirección neta se informa aparte.\n"
    )

    # Transformación independiente: no usa las series P para construir T.
    Tdf=_R_estado(df,["E","I","X","K","K_patentes","conexion"],h=1)
    if Tdf.empty:
        reporte.append("**H3-v8: SIN DATOS de estado suficientes.**")
        return {"version":"H3-v8","estado":"SIN DATOS","familias":{}}
    T=Tdf.set_index("anio")["R"].astype(float).replace([np.inf,-np.inf],np.nan).dropna()
    direccion_bruta=_dacel_direccion_estado(df)
    # Dirección relativa al comportamiento reciente: evita llamar "expansión" a
    # cualquier año sólo porque conocimiento/conectividad tienen tendencia secular.
    base_dir=direccion_bruta.shift(1).rolling(5,min_periods=3).median()
    direccion=(direccion_bruta-base_dir).dropna()

    familias, auditoria = _familias_perturbacion()

    # Tolerancia exclusivamente numérica: no es umbral estadístico ni de significancia.
    escala=float(np.nanmedian(np.abs(T.to_numpy()))) if len(T) else 0.0
    tol=max(np.finfo(float).eps*100, escala*1e-12)

    resultados={}
    total_obs=total_cambio=0
    reporte.append("### Evidencia observada por familia")
    for nombre,P in familias.items():
        P=P.replace([np.inf,-np.inf],np.nan).dropna()
        # Solo años en que ESTA familia tuvo perturbación alta (percentil ex-ante >= 0.75).
        # Así cada familia se evalúa con sus propios años y no con todos los años con datos.
        altos=P[P>=0.75].index
        idx=altos.intersection(T.index).sort_values()
        if len(idx)==0:
            resultados[nombre]={"estado":"SIN DATOS","n":0}
            reporte.append(f"- {nombre}: **SIN DATOS COMPARABLES**")
            continue

        vals=T.loc[idx]
        cambio=vals>tol
        n=int(len(idx)); nc=int(cambio.sum())
        total_obs += n; total_cambio += nc

        d= direccion.reindex(idx)
        exp=int((d>tol).sum())
        con=int((d<-tol).sum())
        neut=int((d.abs()<=tol).sum())
        sindir=int(d.isna().sum())

        # Persistencia: transformación observable durante los 3 pasos siguientes
        persistencias=[]
        for y in idx:
            fut=T.reindex([y,y+1,y+2]).dropna()
            if len(fut):
                persistencias.append(float((fut>tol).mean()))
        persist=float(np.mean(persistencias)) if persistencias else None

        res={
            "estado":"TRANSFORMACION_OBSERVADA_EN_TODOS_LOS_CASOS" if nc==n else "TRANSFORMACION_NO_OBSERVADA_EN_ALGUNOS_CASOS",
            "n_perturbaciones_con_respuesta_observable":n,
            "n_transformacion_observable":nc,
            "proporcion_transformacion":float(nc/n),
            "T_mediana":float(vals.median()),
            "T_media":float(vals.mean()),
            "T_max":float(vals.max()),
            "persistencia_3_pasos":persist,
            "tolerancia_numerica":float(tol),
            "direccion_relativa":{"expansion_acelerada":exp,"contraccion_relativa":con,
                                  "cambio_relativo_casi_nulo":neut,"sin_direccion":sindir},
            "desde":int(idx.min()),"hasta":int(idx.max())
        }
        resultados[nombre]=res
        linea=(
            f"- {nombre}: {nc}/{n} años de perturbación alta ({100*nc/n:.1f}%) muestran "
            f"transformación estructural medible; T mediana={vals.median():.4f}; "
            f"dirección relativa: expansión/aceleración {exp}, "
            f"contracción/desaceleración {con}, casi nula {neut}"
        )
        if sindir:
            linea += f", sin dirección {sindir}"
        if persist is not None:
            linea += f"; persistencia 3 pasos={persist:.1%}"
        linea += "."
        reporte.append(linea)

    prop=(total_cambio/total_obs) if total_obs else np.nan
    supera_h3 = bool(total_obs > 0 and total_cambio == total_obs)

    reporte.append("\n### Calificación del postulado base")
    if total_obs:
        reporte.append(
            f"- Cobertura observada: {total_cambio}/{total_obs} "
            f"({100*prop:.1f}%) años de perturbación alta presentan cambio estructural "
            "medible en el paso anual."
        )
        if supera_h3:
            reporte.append(
                f"**H3 postulado base: {VEREDICTO_CUMPLE} ({100*prop:.1f}%).** "
                f"Los datos reales muestran transformación en {total_cambio} de {total_obs} "
                "años de perturbación alta."
            )
        else:
            reporte.append(
                f"**H3 postulado base: {VEREDICTO_NOCUMPLE} ({100*prop:.1f}%).** "
                f"Hubo {total_obs-total_cambio} años de perturbación alta sin transformación medible."
            )
    else:
        reporte.append(f"**H3 postulado base: {VEREDICTO_NC}.** Sin años comparables.")
    reporte.append(
        "- Este resultado mide **compatibilidad observacional con el postulado Daçel**, "
        "no beneficio moral, progreso ni causalidad estadística. Una contracción o "
        "destrucción sigue siendo transformación."
    )
    reporte.append(
        "- Historial: H3-v2 a v5 no superaron sus pruebas y se conservan como registro. "
        "H3-v8 se reformuló como postulado base el 17-sep-2026. H3-v9 es la prueba de intensidad."
    )

    paquete={"version":"H3-base (antes v8)","tipo":"postulado base",
             "paso_anual":1,"tolerancia_numerica":tol,
             "observaciones_totales":total_obs,
             "transformaciones_observadas":total_cambio,
             "proporcion_global":float(prop) if total_obs else None,
             "cumple_postulado":bool(total_obs>0 and total_cambio==total_obs),"calificacion_pct":float(100*prop) if total_obs else None,"es_postulado_base":True,
             "criterio_superacion":"100% de observaciones comparables con transformación estructural medible",
             "familias":resultados,"auditoria":auditoria}
    with open(os.path.join(carpeta,"H3_v8_estructural.json"),"w",encoding="utf-8") as f:
        json.dump(paquete,f,ensure_ascii=False,indent=2)
    return paquete


def _sin_tendencia(s):
    """Quita la tendencia lineal en el tiempo para evitar correlaciones espurias."""
    s = s.dropna()
    t = s.index.to_numpy(float)
    b = np.polyfit(t, s.to_numpy(float), 1)
    return s - np.polyval(b, t)


def _desvio_trayectoria(df, cols=("E", "I", "X", "K", "K_patentes", "conexion"), ventana=5):
    """
    T(t) = cuánto se aparta el crecimiento de t a t+1 de su trayectoria reciente.
    Para cada variable: |g(t) − mediana de g en los 5 años previos|, dividido entre
    la desviación típica de esos desvíos; luego raíz del promedio de cuadrados.
    Acelerar o frenar cuentan igual: es magnitud de reorganización, no progreso.
    """
    z = df.set_index("anio").sort_index()
    partes = []
    for c in cols:
        if c not in z:
            continue
        ...
        x = x.where(x > 0)
        g = np.log(x).shift(-1) - np.log(x)          # crecimiento de t a t+1
        ref = g.shift(1).rolling(ventana, min_periods=3).median()
        dev = (g - ref).abs()
        sd = dev.std()
        if np.isfinite(sd) and sd > 0 and dev.notna().sum() >= 10:
            partes.append((dev / sd).rename(c))
    if not partes:
        return pd.Series(dtype=float)
    m = pd.concat(partes, axis=1)
    return np.sqrt((m ** 2).mean(axis=1, skipna=True))[m.notna().sum(axis=1) >= 2].dropna()


def prueba_h3_v9_contraste(df, reporte, carpeta, etiqueta, min_n=20):
    """
    H3-v9 — registrada el 17-sep-2026, ANTES de correrse con datos reales.
    Es la formulación principal de H3 sea cual sea su resultado.

    Pregunta: ¿los años con más perturbación van seguidos de MÁS transformación
    estructural que los años con poca perturbación?

      T(t)  = desvío del crecimiento de t a t+1 respecto a su trayectoria reciente
              (acelerar o frenar cuentan igual)
      P(t)  = promedio de las familias de perturbación (percentiles ex-ante),
              con al menos 2 familias disponibles
      Estadístico: Spearman entre P y T, ambas sin tendencia lineal.
      Nulo: todos los desplazamientos circulares de P (conserva autocorrelación).
      Veredicto: rho > 0 y p < 0.05 (unilateral). Familias: solo informativo.
    """
    reporte.append("## H3-v9 — Perturbación contra años normales (prueba vigente)\n")
    reporte.append("Registrada el 17-sep-2026 antes de correrse con datos reales. Pregunta si "
                   "después de los años más perturbados hay más transformación que después de "
                   "años tranquilos. La transformación se mide como cuánto se aparta el crecimiento de su trayectoria reciente, hacia arriba o hacia abajo. Ambas series se analizan sin tendencia para que el simple "
                   "paso del tiempo no genere la relación.\n")
    T = _desvio_trayectoria(df).replace([np.inf, -np.inf], np.nan).dropna()
    if T.empty:
        reporte.append(f"**Veredicto H3-v9: {VEREDICTO_NC}.** Sin datos de estado.\n")
        return VEREDICTO_NC, {}
    familias, _ = _familias_perturbacion()
    familias = {k: v.replace([np.inf, -np.inf], np.nan).dropna()
                for k, v in familias.items() if len(v)}
    detalle = {}

    def contraste(P, nombre):
        idx = P.index.intersection(T.index).sort_values()
        if len(idx) < min_n:
            return None
        p_d, t_d = _sin_tendencia(P.loc[idx]), _sin_tendencia(T.loc[idx])
        rho = lambda x, y: stats.spearmanr(x, y)[0]
        obs, pval, _ = _p_circular(p_d.to_numpy(), t_d.to_numpy(), rho)
        alto = T.loc[idx][P.loc[idx] >= 0.75]
        bajo = T.loc[idx][P.loc[idx] < 0.50]
        r = {"n": int(len(idx)), "desde": int(idx.min()), "hasta": int(idx.max()),
             "rho": float(obs), "p": float(pval),
             "T_mediana_perturbados": float(alto.median()) if len(alto) else None,
             "T_mediana_tranquilos": float(bajo.median()) if len(bajo) else None,
             "n_perturbados": int(len(alto)), "n_tranquilos": int(len(bajo))}
        detalle[nombre] = r
        return r

    if len(familias) >= 2:
        tabla = pd.concat(familias, axis=1)
        conteo = tabla.notna().sum(axis=1)
        P_total = tabla.mean(axis=1, skipna=True)[conteo >= 2]
    else:
        P_total = pd.Series(dtype=float)
    principal = contraste(P_total, "conjunto") if len(P_total) else None

    if principal is None:
        reporte.append(f"**Veredicto H3-v9: {VEREDICTO_NC}.** Menos de {min_n} años con "
                       "perturbación (2+ familias) y transformación a la vez.\n")
        v = VEREDICTO_NC
    else:
        r = principal
        reporte.append(f"- Años analizados: {r['n']} ({r['desde']}–{r['hasta']})")
        reporte.append(f"- Correlación perturbación → transformación (sin tendencia): rho = {r['rho']:+.3f}")
        reporte.append(f"- Nulo con {r['n']-1} desplazamientos circulares: p = {r['p']:.4f}")
        if r["T_mediana_perturbados"] is not None and r["T_mediana_tranquilos"] is not None:
            reporte.append(f"- Transformación mediana tras años muy perturbados ({r['n_perturbados']}): "
                           f"{r['T_mediana_perturbados']:.4f}; tras años tranquilos ({r['n_tranquilos']}): "
                           f"{r['T_mediana_tranquilos']:.4f}")
        if r["rho"] > 0 and r["p"] < ALFA:
            v = VEREDICTO_OK
            txt = "Tras los años más perturbados hay más transformación que tras años tranquilos."
        else:
            v = VEREDICTO_NO
            txt = ("La transformación tras años perturbados no se distingue de la de años "
                   "tranquilos una vez quitada la tendencia.")
        reporte.append(f"\n**Veredicto H3-v9: {v}.** {txt}\n")

    reporte.append("Por familia (solo informativo; con 6 familias se esperan resultados "
                   "\"significativos\" por azar, así que no deciden el veredicto):\n")
    for nombre, P in familias.items():
        r = contraste(P, nombre)
        if r is None:
            reporte.append(f"- {nombre}: menos de {min_n} años comparables.")
        else:
            reporte.append(f"- {nombre}: n = {r['n']}, rho = {r['rho']:+.3f}, p = {r['p']:.3f}")
    reporte.append("")
    with open(os.path.join(carpeta, "H3_v9_contraste.json"), "w", encoding="utf-8") as f:
        json.dump({"version": "H3-v9", "registrada": "2026-09-17", "veredicto": v,
                   "resultados": detalle}, f, ensure_ascii=False, indent=2)
    return v, detalle
...
def prueba_h3_v7_replicacion(h3v6,reporte,carpeta):
    ...
    reporte.append(
        f"- Con señal según criterio preregistrado: {len(positivas)}/{len(evaluables)} "
        f"entre evaluables; {len(positivas)}/{len(r)} del universo preregistrado"
    )
    reporte.append(
        f"- Sin señal: {len(negativas)}/{len(evaluables)} entre evaluables; "
        f"{len(negativas)}/{len(r)} del universo preregistrado"
    )
...
# ============================================================
# H5 — IA y vacío humano (registrada el 17-sep-2026)
# ============================================================

URLS_IA = [
    ("owid", "https://ourworldindata.org/grapher/affiliation-researchers-building-artificial-intelligence-systems-all.csv?v=1&csvType=full&useColumnShortNames=false"),
    ("epoch", "https://epoch.ai/data/notable_ai_models.csv"),
]
CACHE_IA = os.path.join("datos_fuente", "ia_sistemas_notables.csv")


def _serie_ia():
    """
    Número de sistemas de IA notables publicados por año (Epoch AI, vía Our World in Data).
    Si la descarga falla, usa la copia guardada de la corrida anterior.
    Se descarta el año en curso porque sus datos están incompletos.
    """
    import urllib.request
    anio_actual = pd.Timestamp.now().year
    errores = []
    for tipo, url in URLS_IA:
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "TeoriaDacel/2.0"})
            with urllib.request.urlopen(req, timeout=60) as r:
                d = pd.read_csv(io.BytesIO(r.read()), low_memory=False)
            if tipo == "owid":
                filas = d[d["Entity"] == "World"] if (d["Entity"] == "World").any() else d
                num = filas.drop(columns=[c for c in ("Entity", "Code", "Year") if c in filas])
                num = num.apply(pd.to_numeric, errors="coerce")
                serie = num.sum(axis=1, min_count=1).groupby(filas["Year"]).sum()
            else:
                col = [c for c in d.columns if "publication date" in c.lower()][0]
                anios = pd.to_datetime(d[col], errors="coerce").dt.year.dropna().astype(int)
                serie = anios.value_counts().sort_index()
            serie = serie[(serie.index < anio_actual)].astype(float)
            if len(serie) < 10:
                raise ValueError("menos de 10 años")
            os.makedirs("datos_fuente", exist_ok=True)
            serie.rename_axis("anio").rename("n").to_csv(CACHE_IA)
            return serie, {"estado": "OK", "fuente": tipo,
                           "anios": [int(serie.index.min()), int(serie.index.max())]}
        except Exception as e:
            errores.append(f"{tipo}: {str(e)[:120]}")
    if os.path.exists(CACHE_IA):
        c = pd.read_csv(CACHE_IA).set_index("anio")["n"].astype(float)
        return c, {"estado": "COPIA GUARDADA", "errores": errores,
                   "anios": [int(c.index.min()), int(c.index.max())]}
    return pd.Series(dtype=float), {"estado": "SIN DATOS", "errores": errores}


def prueba_h5(df, reporte, carpeta, etiqueta, min_n=20):
    """
    H5 — registrada el 17-sep-2026, antes de correrse con datos reales.
    El crecimiento de la IA aumenta el vacío humano, MÁS ALLÁ de lo que ya explica internet.

      IA(t)   = log(1 + sistemas de IA notables publicados en el año t)
      H(t)    = parte psicológica del HDF (ansiedad, depresión, soledad disponibles)
      Estadístico: correlación parcial entre ΔIA(t) y ΔH(t+1),
                   descontando el efecto de Δconexión(t) (usuarios de internet).
      Rezago de 1 año: el efecto de la tecnología sobre las personas no es inmediato.
      Nulo: todos los desplazamientos circulares de ΔIA.
      Supera si r > 0 y p < 0.05. Mínimo 20 años.
    """
    reporte.append("## H5 — La IA aumenta el vacío humano (más allá de internet)\n")
    reporte.append("Registrada el 17-sep-2026 antes de correrse con datos reales. Pregunta si en los "
                   "años en que la IA crece más rápido, el año siguiente sube más la ansiedad o "
                   "depresión mundial, después de descontar el efecto del crecimiento de internet "
                   "(que ya mide H2).\n")
    ia, meta = _serie_ia()
    h = calcular_hdf_psicologico(df)
    c = df.set_index("anio")["conexion"].dropna() if "conexion" in df else pd.Series(dtype=float)
    if ia.empty or h.empty or c.empty:
        falta = [n for n, x in (("IA", ia), ("vacío humano", h), ("conexión", c)) if x.empty]
        reporte.append(f"**Veredicto H5: {VEREDICTO_NC}.** Faltan datos de: {', '.join(falta)}.\n")
        return VEREDICTO_NC, {"fuente_ia": meta}
    tabla = pd.DataFrame({"ia": np.log1p(ia), "h": h, "c": c}).sort_index()
    tabla.index = tabla.index.astype(int)
    tabla = tabla.reindex(range(tabla.index.min(), tabla.index.max() + 1))
    d = pd.DataFrame({
        "dia": tabla["ia"].diff(),
        "dc": tabla["c"].diff(),
        "dh_sig": tabla["h"].diff().shift(-1),
    }).dropna()
    if len(d) < min_n:
        reporte.append(f"- Años con IA, vacío humano y conexión a la vez: {len(d)}")
        reporte.append(f"**Veredicto H5: {VEREDICTO_NC}.** Se necesitan al menos {min_n}.\n")
        return VEREDICTO_NC, {"fuente_ia": meta, "n": int(len(d))}
    ...
    r_ia = residuo(d["dia"].to_numpy(), d["dc"].to_numpy())
    r_h = residuo(d["dh_sig"].to_numpy(), d["dc"].to_numpy())
    corr = lambda x, y: np.corrcoef(x, y)[0, 1]
    obs, p, _ = _p_circular(r_ia, r_h, corr)
    simple = corr(d["dia"], d["dh_sig"])

    reporte.append(f"- Años analizados: {len(d)} ({int(d.index.min())}–{int(d.index.max())})")
    reporte.append(f"- Fuente de IA: {meta.get('fuente', meta.get('estado'))}")
    reporte.append(f"- Correlación simple (sin descontar internet): r = {simple:+.3f}")
    reporte.append(f"- Correlación parcial (descontando internet): r = {obs:+.3f}")
    reporte.append(f"- Nulo con {len(d)-1} desplazamientos circulares: p = {p:.4f}")
    if obs > 0 and p < ALFA:
        v, txt = VEREDICTO_OK, ("Cuando la IA acelera, el año siguiente el vacío humano sube más, "
                                "y no se explica solo por internet.")
    elif obs > 0:
        v, txt = VEREDICTO_NO, ("La dirección es la prevista, pero la relación no se distingue del azar "
                                "con los años disponibles.")
    else:
        v, txt = VEREDICTO_NO, "No aparece la relación prevista una vez descontado internet."
    reporte.append(f"\n**Veredicto H5: {v}.** {txt}\n")
    reporte.append("Límite actual: las series mundiales de salud mental suelen terminar años antes del "
                   "presente, y antes de 2022 la IA llegaba poco al público. El periodo de IA masiva "
                   "entrará a la prueba conforme se publiquen nuevos datos.\n")

    fig, ax = plt.subplots(figsize=(7, 5))
    ax.scatter(r_ia, r_h, color="k", s=18)
    ax.set_title(f"H5 — IA y vacío humano, descontando internet ({etiqueta})")
    ax.set_xlabel("aceleración de la IA (residuo)")
    ax.set_ylabel("cambio del vacío humano al año siguiente (residuo)")
    fig.tight_layout(); fig.savefig(os.path.join(carpeta, "H5_IA_vacio.png"), dpi=130); plt.close(fig)
    res = {"n": int(len(d)), "r_parcial": float(obs), "p": float(p), "r_simple": float(simple),
           "fuente_ia": meta, "veredicto": v, "registrada": "2026-09-17"}
    with open(os.path.join(carpeta, "H5_IA.json"), "w", encoding="utf-8") as f:
...
# ============================================================
# PRUEBAS POR PAÍSES (registradas el 17-sep-2026)
# ============================================================

def _demean_dos_vias(valores, g_pais, g_anio, iteraciones=30):
    """Quita el promedio de cada país y de cada año (efectos fijos de dos vías).
    Funciona con paneles incompletos mediante proyecciones alternadas."""
    x = np.asarray(valores, float).copy()
    np_, na_ = g_pais.max() + 1, g_anio.max() + 1
    cp, ca = np.bincount(g_pais, minlength=np_), np.bincount(g_anio, minlength=na_)
    for _ in range(iteraciones):
        x -= (np.bincount(g_pais, x, np_) / cp)[g_pais]
        x -= (np.bincount(g_anio, x, na_) / ca)[g_anio]
    return x


MAX_HUECO_INTERNET = 2        # años seguidos que se pueden rellenar dentro de una serie
COBERTURA_PERIODO = 0.80      # fracción de países con datos que debe quedar completa
MIN_ANIOS_CAMBIO = 10         # años de cambio mínimos del periodo común


def _rellenar_huecos(anios, valores, max_hueco=MAX_HUECO_INTERNET):
    """Interpolación lineal SOLO en huecos internos de hasta `max_hueco` años seguidos.
    Nunca rellena antes del primer dato, después del último, ni huecos más largos."""
    v = np.asarray(valores, float).copy()
    a = np.asarray(anios, float)
    ok = np.where(np.isfinite(v))[0]
    for i, j in zip(ok[:-1], ok[1:]):
        hueco = j - i - 1
        if 1 <= hueco <= max_hueco:
            v[i + 1:j] = v[i] + (v[j] - v[i]) * (a[i + 1:j] - a[i]) / (a[j] - a[i])
    return v


def _preparar_panel(panel):
    """Rejilla completa país × año (años de salud mental) y relleno de huecos cortos de internet."""
    p = panel.copy()
    p["anio"] = p["anio"].astype(int)
    con_salud = p.dropna(subset=["A", "Dp"])
    anios = range(int(con_salud["anio"].min()), int(con_salud["anio"].max()) + 1)
    rejilla = pd.MultiIndex.from_product([sorted(p["iso3"].unique()), anios], names=["iso3", "anio"])
    p = p.drop_duplicates(["iso3", "anio"]).set_index(["iso3", "anio"]).reindex(rejilla).reset_index()
    p["conexion_original"] = p["conexion"]
    partes = []
    for _, g in p.groupby("iso3", sort=True):
        g = g.sort_values("anio").copy()
        g["conexion"] = _rellenar_huecos(g["anio"].to_numpy(), g["conexion"].to_numpy())
        partes.append(g)
    p = pd.concat(partes, ignore_index=True)
    p["conexion_rellenada"] = p["conexion"].notna() & p["conexion_original"].isna()
    return p


def _periodo_comun(p):
    """Elige el inicio más temprano en que al menos el 80% de los países con datos de internet
    tiene serie completa (salud mental e internet) hasta el último año de salud mental."""
    fin = int(p.dropna(subset=["A", "Dp"])["anio"].max())
    con_datos = p.dropna(subset=["conexion"])["iso3"].nunique()
    completo = p["A"].notna() & p["Dp"].notna() & p["conexion"].notna()
    for inicio in sorted(p["anio"].unique()):
        if fin - inicio < MIN_ANIOS_CAMBIO:
            break
        ventana = p[(p["anio"] >= inicio) & (p["anio"] <= fin)]
        ok = completo[ventana.index].groupby(ventana["iso3"]).all()
        if con_datos and ok.sum() >= COBERTURA_PERIODO * con_datos:
            return int(inicio), fin, sorted(ok[ok].index), int(con_datos)
    return None, fin, [], int(con_datos)
...
def _cambios_panel(panel, min_anios=15):
    """Cambios anuales por país: dH = promedio de ΔA y ΔDp (puntos porcentuales),
    dC = Δ usuarios de internet (puntos porcentuales). Solo pares de años consecutivos."""
    p = panel.sort_values(["iso3", "anio"]).copy()
    p["H"] = p[["A", "Dp"]].mean(axis=1)
    g = p.groupby("iso3")
    consecutivo = g["anio"].diff() == 1
    p["dH"] = g["H"].diff().where(consecutivo)
    p["dC"] = g["conexion"].diff().where(consecutivo)
    d = p.dropna(subset=["dH", "dC"])
    cuenta = d.groupby("iso3")["anio"].transform("size")
    return d[cuenta >= min_anios].reset_index(drop=True)


def prueba_h2_paises(panel, reporte, carpeta, n_perm=1000, semilla=20260917):
    """
    H2-países — registrada el 17-sep-2026, antes de correrse con datos reales.
      En cada país, ¿los años en que internet crece más son también los años en que
      más sube la ansiedad/depresión?
      Estadístico: pendiente β de dH sobre dC con efectos fijos de país y de año
      (quita tendencias propias de cada país y cualquier choque mundial común).
      Nulo: asignar a cada país la historia de internet de otro país al azar (1000 veces).
      Panel balanceado: huecos internos de internet de 1–2 años se rellenan en línea recta;
      el periodo empieza en el primer año en que al menos el 80% de los países con datos
      de internet tiene serie completa (mínimo 10 años de cambios).
      Cada país pesa igual. Mínimo: 50 países.
      Supera si β > 0 y p < 0.05.
    """
    reporte.append("## H2-países — El vacío humano sube donde y cuando crece internet\n")
    reporte.append("Registrada el 17-sep-2026 antes de correrse con datos reales. Compara, dentro de "
                   "cada país, los años de mayor crecimiento de internet con los cambios de ansiedad "
                   "y depresión. Se descuentan las tendencias propias de cada país y cualquier "
                   "cambio que afecte a todo el mundo el mismo año.\n")
    p = _preparar_panel(panel)
    inicio, fin, paises_ok, con_datos = _periodo_comun(p)
    if inicio is None:
        reporte.append(f"**Veredicto H2-países: {VEREDICTO_NC}.** Ningún periodo de al menos "
                       f"{MIN_ANIOS_CAMBIO} años deja completo al {int(COBERTURA_PERIODO*100)}% de los países.\n")
        return VEREDICTO_NC, {"paises": 0}
    v = p[p["iso3"].isin(paises_ok) & (p["anio"] >= inicio) & (p["anio"] <= fin)]
    v = v.sort_values(["iso3", "anio"]).copy()
    v["H"] = v[["A", "Dp"]].mean(axis=1)
    g = v.groupby("iso3")
    v["dH"] = g["H"].diff()
    v["dC"] = g["conexion"].diff()
    d = v.dropna(subset=["dH", "dC"]).reset_index(drop=True)
    n_paises = d["iso3"].nunique() if len(d) else 0
    rellenos = int(v["conexion_rellenada"].sum())
    reporte.append(f"- Periodo común: {inicio}–{fin} (cambios {inicio+1}–{fin}); países completos: "
                   f"{n_paises} de {con_datos} con datos de internet; valores de internet rellenados "
                   f"por huecos de 1–{MAX_HUECO_INTERNET} años: {rellenos} de {len(v)}.")
    if n_paises < 50:
        reporte.append(f"**Veredicto H2-países: {VEREDICTO_NC}.** Solo {n_paises} países completos; se necesitan 50.\n")
        return VEREDICTO_NC, {"paises": int(n_paises)}
    gp = d["iso3"].astype("category").cat.codes.to_numpy()
    ga = d["anio"].astype("category").cat.codes.to_numpy()
    y = _demean_dos_vias(d["dH"].to_numpy(), gp, ga)
    obs = _beta(_demean_dos_vias(d["dC"].to_numpy(), gp, ga), y)
    if not np.isfinite(obs):
        reporte.append(f"**Veredicto H2-países: {VEREDICTO_NC}.** El crecimiento de internet no varía entre países y años una vez descontadas las tendencias.\n")
        return VEREDICTO_NC, {"paises": int(n_paises)}

    # Panel balanceado: se ordena por país y año; todos los países tienen los mismos años.
    n_anios = d["anio"].nunique()
    tamanos = d.groupby("iso3").size()
    if not ((tamanos == n_anios).all() and d["iso3"].is_monotonic_increasing
            and len(d) == n_paises * n_anios):
        raise RuntimeError("H2-países: el panel no quedó balanceado; revisar datos.")
    rng = np.random.default_rng(semilla)
    matriz_dC = d["dC"].to_numpy().reshape(n_paises, n_anios)
    nulos = np.empty(n_perm)
    for r in range(n_perm):
        # Nulo: a cada país se le asigna la historia de internet de otro país al azar.
        z = matriz_dC[rng.permutation(n_paises)].ravel()
        nulos[r] = _beta(_demean_dos_vias(z, gp, ga), y)
    nulos = nulos[np.isfinite(nulos)]
    pval = (np.sum(nulos >= obs) + 1) / (len(nulos) + 1)
    positivos = d.groupby("iso3").apply(
        lambda t: np.corrcoef(t["dC"], t["dH"])[0, 1] if t["dC"].std() > 0 and t["dH"].std() > 0 else np.nan,
        include_groups=False).dropna()

    reporte.append(f"- Observaciones país-año: {len(d)} "
                   f"({int(d['anio'].min())}–{int(d['anio'].max())})")
    reporte.append(f"- Pendiente β: {obs:+.5f} puntos de vacío humano por cada punto de internet")
    reporte.append(f"- Nulo con {n_perm} intercambios de historias entre países: p = {pval:.4f}")
    reporte.append(f"- Países con relación positiva dentro de su propia historia: "
                   f"{int((positivos > 0).sum())} de {len(positivos)} (descriptivo)")
    if obs > 0 and pval < ALFA:
        veredicto, txt = VEREDICTO_OK, "Dentro de cada país, los años de mayor crecimiento de internet acompañan mayores aumentos de ansiedad y depresión."
    elif obs > 0:
        veredicto, txt = VEREDICTO_NO, "La dirección es la prevista, pero no se distingue del azar."
    else:
        veredicto, txt = VEREDICTO_NO, "Dentro de cada país no aparece la relación prevista."
    reporte.append(f"\n**Veredicto H2-países: {veredicto}.** {txt}\n")
    reporte.append("Límite: las cifras por país del IHME son estimaciones de un modelo, suaves y en parte "
                   "construidas con información de países vecinos; eso puede influir en la prueba.\n")

    fig, ax = plt.subplots(figsize=(7, 5))
    ax.hist(nulos, bins=40, color="lightgray", label="historias intercambiadas al azar")
    ax.axvline(obs, color="red", lw=2, label=f"observado β={obs:+.4f}")
    ax.set_title("H2-países — internet y vacío humano dentro de cada país")
    ax.set_xlabel("pendiente β"); ax.legend()
    fig.tight_layout(); fig.savefig(os.path.join(carpeta, "H2_paises.png"), dpi=130); plt.close(fig)
    res = {"veredicto": veredicto, "beta": obs, "p": float(pval), "paises": int(n_paises),
           "periodo": [inicio, fin], "valores_rellenados": rellenos,
           "observaciones": int(len(d)), "paises_positivos": int((positivos > 0).sum()),
           "paises_evaluados": int(len(positivos)), "registrada": "2026-09-17"}
    with open(os.path.join(carpeta, "H2_paises.json"), "w", encoding="utf-8") as f:
        json.dump(res, f, ensure_ascii=False, indent=2)
    return veredicto, res


ARCHIVO_IA_PAISES = os.path.join("datos_fuente", "ia_uso_paises.csv")


def prueba_h5_paises(panel, reporte, carpeta, n_perm=1000, semilla=20260918):
    """
    H5-países — registrada el 17-sep-2026. Se activa sola cuando exista
    datos_fuente/ia_uso_paises.csv con columnas iso3, anio, ia_pct
    (% de población que usa IA generativa, de una fuente documentada).
      Estadístico: pendiente de dH(t+1) sobre dIA(t), descontando dC(t),
      con efectos fijos de país y de año.
      Nulo: permutar dIA entre países dentro del mismo año (1000 veces).
      Mínimo: 50 países con al menos 2 cambios válidos.
      Supera si la pendiente > 0 y p < 0.05.
    """
    reporte.append("## H5-países — La IA aumenta el vacío humano, país por país\n")
    reporte.append("Registrada el 17-sep-2026. Diseño fijo: se activa cuando existan datos de uso de "
                   "IA por país y años de salud mental que los cubran.\n")
    if not os.path.exists(ARCHIVO_IA_PAISES):
        reporte.append(f"**Veredicto H5-países: {VEREDICTO_NC}.** Registrada; esperando datos de uso de IA por país "
                       f"({ARCHIVO_IA_PAISES}).\n")
        return VEREDICTO_NC, {"estado": "esperando datos"}
    ia = pd.read_csv(ARCHIVO_IA_PAISES)[["iso3", "anio", "ia_pct"]]
    p = _preparar_panel(panel).merge(ia, on=["iso3", "anio"], how="left").sort_values(["iso3", "anio"])
    p["H"] = p[["A", "Dp"]].mean(axis=1)
    g = p.groupby("iso3")
    cons = g["anio"].diff() == 1
    p["dIA"] = g["ia_pct"].diff().where(cons)
    p["dC"] = g["conexion"].diff().where(cons)
    sig = g["anio"].shift(-1) == p["anio"] + 1
    p["dH_sig"] = (g["H"].shift(-1) - p["H"]).where(sig)
    d = p.dropna(subset=["dIA", "dC", "dH_sig"])
    d = d[d.groupby("iso3")["anio"].transform("size") >= 2].reset_index(drop=True)
    n_paises = d["iso3"].nunique() if len(d) else 0
    if n_paises < 50:
        reporte.append(f"- Países con datos suficientes: {n_paises}")
        reporte.append(f"**Veredicto H5-países: {VEREDICTO_NC}.** Se necesitan 50 países con al menos 2 cambios válidos.\n")
        return VEREDICTO_NC, {"paises": int(n_paises)}
    gp = d["iso3"].astype("category").cat.codes.to_numpy()
    ga = d["anio"].astype("category").cat.codes.to_numpy()
    c = _demean_dos_vias(d["dC"].to_numpy(), gp, ga)
    def residuo(v):
        v = _demean_dos_vias(v, gp, ga)
        return v - _beta(c, v) * c
    y = residuo(d["dH_sig"].to_numpy())
    obs = _beta(residuo(d["dIA"].to_numpy()), y)
    if not np.isfinite(obs):
        reporte.append(f"**Veredicto H5-países: {VEREDICTO_NC}.** El uso de IA no varía lo suficiente entre países y años.\n")
        return VEREDICTO_NC, {"paises": int(n_paises)}
    rng = np.random.default_rng(semilla)
    por_anio = [np.where(ga == k)[0] for k in range(ga.max() + 1)]
    dIA = d["dIA"].to_numpy()
    nulos = np.empty(n_perm)
    for r in range(n_perm):
        z = dIA.copy()
        for idx in por_anio:
            z[idx] = dIA[rng.permutation(idx)]
        nulos[r] = _beta(residuo(z), y)
    nulos = nulos[np.isfinite(nulos)]
    pval = (np.sum(nulos >= obs) + 1) / (len(nulos) + 1)
    reporte.append(f"- Países: {n_paises}; observaciones: {len(d)}")
    reporte.append(f"- Pendiente (descontando internet): {obs:+.5f}")
    reporte.append(f"- Nulo con {n_perm} permutaciones: p = {pval:.4f}")
    v = VEREDICTO_OK if (obs > 0 and pval < ALFA) else VEREDICTO_NO
    reporte.append(f"\n**Veredicto H5-países: {v}.**\n")
    return v, {"veredicto": v, "beta": obs, "p": float(pval), "paises": int(n_paises)}
...
def prueba_dacel_multiescala(df,reporte,carpeta,etiqueta):
    ...
    try:
        ...
        reporte.append(f"### Biológico\nWHO aporta una serie de transformación biológica de {len(pbi)} años. Aporta una serie biológica independiente para la medición estructural; no se interpreta mortalidad como progreso.\n")
    except Exception as e:
...
def ejecutar(df, etiqueta, carpeta, semilla=42):
    ...
    veredictos = {
        ...
        # H3-v4/v5 se ejecutan arriba como auditoría histórica, pero no forman parte del resumen vigente.
        "_auditoria_H3_v4": prueba_h3(df, reporte, carpeta, etiqueta, rng_h3),
        "_auditoria_H3_v5": prueba_h3_v5_historica(df, reporte, carpeta, etiqueta),
        "H4 (externalización 2040)": prueba_h4(df, reporte),
        "Ecuación General": prueba_ecuacion_general(df, reporte, carpeta, etiqueta),
    }
    # Formulación vigente: H3-v8 estructural. Las versiones v4/v5/v6/v7
    # permanecen en el historial del repositorio, pero no gobiernan H3 actual.
    auditoria_datos_maestros(reporte, carpeta)
    multiescala = prueba_dacel_multiescala(df, reporte, carpeta, etiqueta)
    h3v8 = prueba_h3_v8_estructural(df, reporte, carpeta, etiqueta)
    h3v9, _ = prueba_h3_v9_contraste(df, reporte, carpeta, etiqueta)
    if h3v8.get("observaciones_totales", 0):
        base = VEREDICTO_CUMPLE if h3v8.get("cumple_postulado") else VEREDICTO_NOCUMPLE
        veredictos["H3 (postulado base)"] = f"{base} ({h3v8['calificacion_pct']:.0f}%)"
    else:
        veredictos["H3 (postulado base)"] = VEREDICTO_NC
    veredictos["H3-v9 (intensidad)"] = h3v9
    h5, h5_det = prueba_h5(df, reporte, carpeta, etiqueta)
    veredictos["H5 (IA y vacío humano)"] = h5
    ruta_panel = os.path.join("datos", "panel_paises.csv")
    if os.path.exists(ruta_panel):
        panel = pd.read_csv(ruta_panel)
        h2p, h2p_det = prueba_h2_paises(panel, reporte, carpeta)
        h5p, h5p_det = prueba_h5_paises(panel, reporte, carpeta)
    else:
        reporte.append("## Pruebas por países\n\nNo existe datos/panel_paises.csv todavía.\n")
        h2p, h2p_det, h5p, h5p_det = VEREDICTO_NC, {}, VEREDICTO_NC, {}
    veredictos["H2-países (vacío humano)"] = h2p
    veredictos["H5-países (IA)"] = h5p
    reporte.append("## Resumen\n")
    for k, v in veredictos.items():
        if not k.startswith("_auditoria_"):
            reporte.append(f"- {k}: **{v}**")
    reporte.append("\nH3 postulado base: verificación con datos del principio de reestructuración "
                   "(calificación = % de años de perturbación alta con transformación medible). "
                   "H3-v9 y H5 fueron registradas el 17-sep-2026 antes de correrse con datos reales. "
                   "H3-v2 a v5 permanecen en el historial.\n")
    ...
    if a.datos:
        ...
        for k in v1:
            if not k.startswith("_"):
                print(f"{k:30s} {str(v1[k]):22s} {str(v2[k]):22s}")
