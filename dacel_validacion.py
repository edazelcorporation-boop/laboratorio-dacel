...
def _buscar_archivo_dacel(patrones, carpeta="datos_fuente"):
    """Busca recursivamente el primer archivo cuyo nombre contenga alguno de los patrones."""
...
def _combinar_canales_exante(canales, min_canales=1):
    ...
    validos=[pd.Series(s,dtype=float).rename(n) for n,s in canales.items() if len(pd.Series(s).dropna())]
    if not validos:
        return pd.Series(dtype=float)
...
def _serie_conflicto_ucdp():
    """UCDP/PRIO: presión anual por conflictos activos, alta intensidad, episodios nuevos y dispersión."""
    ...
    cy=cmap.get("year")
    if cy is None:
        raise ValueError("UCDP no contiene columna year")
    d["__anio"]=_anio_numerico(d[cy])
    d=d.dropna(subset=["__anio"]); d["__anio"]=d["__anio"].astype(int)
    cid=cmap.get("conflict_id")
    cint=cmap.get("intensity_level")
    cloc=cmap.get("location")
    cstart=cmap.get("start_date") or cmap.get("start_date2")
    g=d.groupby("__anio")
    activos=g[cid].nunique() if cid else g.size()
    if cint:
        intensidad=pd.to_numeric(d[cint],errors="coerce")
        altos=d.assign(__hi=(intensidad>=2).astype(float)).groupby("__anio")["__hi"].sum()
    else:
        altos=pd.Series(index=activos.index,dtype=float)
    if cloc:
        dispersion=g[cloc].nunique()
    else:
        dispersion=pd.Series(index=activos.index,dtype=float)
    if cstart:
        ...
        # Alternativa reproducible: primera aparición de cada conflict_id.
        if cid:
            ...
            nuevos=pd.Series(index=activos.index,dtype=float)
    canales={
        "activos":_percentil_exante_min(np.log1p(activos)),
...
def _serie_muertes_batalla():
    """Canal adicional de severidad bélica; no reemplaza cobertura UCDP 1946+."""
    ruta=_buscar_archivo_dacel(["battledeaths","battle_deaths"])
    if not ruta:
        return pd.Series(dtype=float), {"estado":"SIN DATOS","fuente":"UCDP Battle Deaths"}
    d=_leer_csv_flexible(ruta); cmap={str(c).lower():c for c in d.columns}
    if "year" not in cmap or "bd_best" not in cmap:
        raise ValueError("BattleDeaths requiere year y bd_best")
    y=_anio_numerico(d[cmap["year"]]); b=pd.to_numeric(d[cmap["bd_best"]],errors="coerce")
    z=pd.DataFrame({"anio":y,"v":b}).dropna()
    s=z.groupby("anio")["v"].sum().astype(float)
    p=_percentil_exante_min(np.log1p(s.clip(lower=0)))
    return p.dropna(),{"estado":"OK","fuente":os.path.basename(ruta),
                       "rango":[int(p.index.min()),int(p.index.max())] if len(p.dropna()) else None}


def _serie_sismos_usgs():
    """
    USGS M>=6: perturbación geofísica anual.
    Magnitud es logarítmica; se usa energía relativa ~10^(1.5*M), reescalada por año
    para evitar overflow. También se conservan conteo y máximo anual.
    """
    ruta=_buscar_archivo_dacel(["query.csv","earthquake","usgs","sismo"])
    if not ruta:
        return pd.Series(dtype=float), {"estado":"SIN DATOS","fuente":"USGS"}
    d=_leer_csv_flexible(ruta); cmap={str(c).lower():c for c in d.columns}
    if "time" not in cmap or "mag" not in cmap:
        raise ValueError("USGS requiere time y mag")
    fecha=pd.to_datetime(d[cmap["time"]],errors="coerce",utc=True)
    mag=pd.to_numeric(d[cmap["mag"]],errors="coerce")
    z=pd.DataFrame({"anio":fecha.dt.year,"mag":mag}).dropna()
    z=z[z.mag>=6.0]
    g=z.groupby("anio")
    conteo=g.size().astype(float)
    maxmag=g.mag.max().astype(float)
    # log10 de suma de energía relativa: log10(sum(10^(1.5M))).
    def logenergia(x):
        a=1.5*np.asarray(x,float)
        m=float(np.max(a))
    ...
    canales={
        ...
        "energia":_percentil_exante_min(energia),
    }
    p=_combinar_canales_exante(canales,min_canales=2)
    return p,{"estado":"OK","fuente":os.path.basename(ruta),
              "rango":[int(p.index.min()),int(p.index.max())] if len(p) else None,
              "nota":"Agregado mundial; no se asigna país a partir del texto place."}


def _leer_wdi_largo():
    """Localiza una exportación WDI y la convierte a formato indicador-país-año."""
    ruta=_buscar_archivo_dacel(["data.csv","world_development","wdi"])
    if not ruta:
        return pd.DataFrame(), {"estado":"SIN DATOS","fuente":"World Bank WDI"}
    d=_leer_csv_flexible(ruta)
    ...
    if ccode is None:
        raise ValueError("WDI: no encuentro Series Code/Indicator Code")
    yearcols=[]
    for c in d.columns:
        m=re.search(r"(19|20)\d{2}",str(c))
        if m:
            yearcols.append((c,int(m.group(0))))
    if not yearcols:
        raise ValueError("WDI: no encuentro columnas de años")
    ids=[ccode]+([cname] if cname else [])+([ciso] if ciso else [])
    q=d[ids+[c for c,_ in yearcols]].copy()
    ren={c:str(y) for c,y in yearcols}; q=q.rename(columns=ren)
    q=q.melt(id_vars=ids,var_name="anio",value_name="valor")
    q["anio"]=pd.to_numeric(q["anio"],errors="coerce")
    q["valor"]=pd.to_numeric(q["valor"],errors="coerce")
    q=q.dropna(subset=["anio","valor"]); q["anio"]=q["anio"].astype(int)
    q=q.rename(columns={ccode:"codigo"})
    if cname: q=q.rename(columns={cname:"pais"})
    if ciso: q=q.rename(columns={ciso:"iso"})
    return q,{"estado":"OK","fuente":os.path.basename(ruta)}


def _wdi_global_por_codigo(wdi,codigo,transformacion="mediana"):
    """
    Agregado anual robusto entre países. No inventa un 'World' si no existe.
    Para crecimiento/inflación se usa mediana; para niveles positivos puede usarse mediana.
    """
    ...
    # Evita mezclar agregados regionales si existe ISO3; conserva códigos de 3 letras
    # pero la mediana robusta reduce el efecto de agregados que hayan quedado.
    return z.groupby("anio")["valor"].median().astype(float).sort_index()
...
def _series_wdi_multicausal():
    ...
    if wdi.empty:
        return {},meta
    # ECONÓMICO: magnitud de disrupción, no dirección.
    eco={}
    for cod in ["NY.GDP.MKTP.KD.ZG","NY.GDP.PCAP.KD.ZG","FP.CPI.TOTL.ZG"]:
        s=_wdi_global_por_codigo(wdi,cod)
        if len(s):
            # desviación causal respecto de mediana de 10 años previos
            dis=pd.Series(index=s.index,dtype=float)
            for a in s.index:
                prev=s.loc[(s.index>=a-10)&(s.index<=a-1)].dropna()
                if len(prev)>=7:
                    dis.loc[a]=abs(float(s.loc[a])-float(prev.median()))
            eco[cod]=_percentil_exante_min(dis)
    Peco=_combinar_canales_exante(eco,min_canales=1)

    # ECOLÓGICO: presión/cambio. Se usan CAMBIOS, no niveles, para no confundir
    # países naturalmente forestales o contaminados con perturbación.
    ecol={}
    for cod in ["AG.LND.FRST.ZS","EN.ATM.CO2E.PC","EN.ATM.PM25.MC.M3","ER.H2O.FWST.ZS"]:
        s=_wdi_global_por_codigo(wdi,cod)
        if len(s)>=3:
            ds=s.diff().abs()
            ecol[cod]=_percentil_exante_min(ds)
    Pecol=_combinar_canales_exante(ecol,min_canales=1)

    # TECNOLÓGICO: aceleración/cambio en adopción y producción tecnológica.
    tec={}
    for cod in ["IT.NET.USER.ZS","IT.CEL.SETS.P2","IP.PAT.RESD",
                "GB.XPD.RSDV.GD.ZS","SP.POP.SCIE.RD.P6","IP.JRN.ARTC.SC",
                "IT.NET.SECR.P6"]:
        s=_wdi_global_por_codigo(wdi,cod)
        if len(s)>=3:
            # log1p para conteos/niveles no negativos; diferencia absoluta = magnitud de cambio
            ss=np.log1p(s.clip(lower=0))
            tec[cod]=_percentil_exante_min(ss.diff().abs())
    Ptec=_combinar_canales_exante(tec,min_canales=1)
    return {"económica":Peco,"ecológica":Pecol,"tecnológica":Ptec},meta


def _serie_biologica_who():
    """
    WHO Mortality Database: deliberadamente NO crea todavía P_biológica a partir de
    mortalidad total. Para una prueba causal se necesita un mapa ICD preregistrado de
    enfermedades transmisibles/exceso de mortalidad y control de cambios ICD/cobertura.
    Detecta los archivos y deja constancia; no fabrica una señal.
    """
    ruta=_buscar_archivo_dacel(["morticd10","morticd09","morticd08","morticd07"])
    if ruta:
        return pd.Series(dtype=float), {
            "estado":"PENDIENTE MAPA ICD",
            "fuente":os.path.basename(ruta),
            "nota":"Archivos WHO detectados; no se mezclan revisiones ICD sin crosswalk preregistrado."
        }
    return pd.Series(dtype=float),{"estado":"SIN DATOS","fuente":"WHO Mortality Database"}
...
def _h3v6_evaluar_familia(P,R,min_n=18):
...
def auditoria_datos_maestros(reporte,carpeta):
    ...
    claves={
        ...
        "USGS sismos":["query.csv","earthquake","usgs","sismo"],
        "WDI":["data.csv","world_development","wdi"],
        "WHO ICD":["morticd10","morticd09","morticd08","morticd07"],
    }
    inv={}
    reporte.append("## Auditoría de datos maestros\n")
    for nombre,pats in claves.items():
        ruta=_buscar_archivo_dacel(pats)
        inv[nombre]=ruta
        reporte.append(f"- {nombre}: {'OK — '+ruta if ruta else 'NO DETECTADO'}")
