...
def _who_anual_desde_archivos():
    """
    Construye señal biológica de transformación a partir de mortalidad WHO.
    No mezcla códigos ICD como si fueran la misma enfermedad: usa mortalidad total anual
    y, cuando existe Cause, diversidad/concentración dentro de cada revisión como canales
    de reorganización. Los chunks son concatenados una sola vez.
    """
    ...
    for ruta in rutas:
        try:
            ...
            year,cause,deaths,country,sex=_who_detectar_columnas(d)
            if year is None or deaths is None:
                errores.append(f"{os.path.basename(ruta)}: sin Year/Deaths"); continue
            z=pd.DataFrame({"anio":_anio_numerico(d[year]),
                            "muertes":pd.to_numeric(d[deaths],errors="coerce")})
            if cause is not None: z["causa"]=d[cause].astype(str)
            if country is not None: z["pais"]=d[country].astype(str)
            if sex is not None: z["sexo"]=d[sex]
            z=z.dropna(subset=["anio","muertes"]); z["anio"]=z["anio"].astype(int)
            # WHO suele contener total y desgloses por edad. Deaths1 es preferente;
            # al trabajar con la columna detectada no sumamos columnas de edad.
    ...
    z=pd.concat(piezas,ignore_index=True)
    # Evita duplicados exactos entre archivos/chunks, sin borrar registros distintos legítimos.
    z=z.drop_duplicates()
    anual=z.groupby("anio")["muertes"].sum().astype(float).rename("muertes").to_frame()
    if "causa" in z:
        anual["causas"]=z.groupby("anio")["causa"].nunique().astype(float)
    if "pais" in z:
        anual["cobertura_paises"]=z.groupby("anio")["pais"].nunique().astype(float)
    return anual.sort_index(),{"estado":"OK","archivos":usados,"errores":errores,
                               "rango":[int(anual.index.min()),int(anual.index.max())]}


def _serie_biologica_who():
    """
    Señal de perturbación/transformación biológica WHO.
    Usa cambios de mortalidad total y, si están disponibles, cambios en diversidad causal.
    La cobertura por países se audita y NO se interpreta como mortalidad.
    """
    anual,meta=_who_anual_desde_archivos()
    if anual.empty: return pd.Series(dtype=float),meta
    canales={}
    if anual["muertes"].notna().sum()>=10:
        canales["cambio_muertes"]=_percentil_exante_min(
            np.log1p(anual["muertes"].clip(lower=0)).diff().abs())
    if "causas" in anual and anual["causas"].notna().sum()>=10:
        canales["cambio_causas"]=_percentil_exante_min(
            np.log1p(anual["causas"].clip(lower=0)).diff().abs())
    p=_combinar_canales_exante(canales,min_canales=1)
    meta["nota"]="Magnitud de cambio biológico; no interpreta muerte como progreso."
    return p,meta
