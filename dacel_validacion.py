...
def prueba_h2_paises(panel, reporte, carpeta, n_perm=1000, semilla=20260917):
    ...
    H2-países — registrada el 17-sep-2026, antes de correrse con datos reales.
      ...
      Panel balanceado: solo países con todos los años del periodo.
    ...
    d = _cambios_panel(panel, min_anios=1)
    if len(d):
        # Periodo común: todos los años de cambio entre el primero y el último con datos de salud mental
        anios = np.arange(int(d["anio"].min()), int(d["anio"].max()) + 1)
        completos = d.groupby("iso3")["anio"].apply(lambda a: set(anios) <= set(a))
        d = d[d["iso3"].isin(completos[completos].index) & d["anio"].isin(anios)]
        d = d.sort_values(["iso3", "anio"]).reset_index(drop=True)
    n_paises = d["iso3"].nunique() if len(d) else 0
    if n_paises < 50:
        reporte.append(f"**Veredicto H2-países: {VEREDICTO_NC}.** Solo {n_paises} países con todos los años; se necesitan 50.\n")
    ...
    if obs > 0 and p < ALFA:
        v, txt = VEREDICTO_OK, "Dentro de cada país, los años de mayor crecimiento de internet acompañan mayores aumentos de ansiedad y depresión."
    elif obs > 0:
        v, txt = VEREDICTO_NO, "La dirección es la prevista, pero no se distingue del azar."
    else:
        v, txt = VEREDICTO_NO, "Dentro de cada país no aparece la relación prevista."
    reporte.append(f"\n**Veredicto H2-países: {v}.** {txt}\n")
    ...
    res = {"veredicto": v, "beta": obs, "p": float(p), "paises": int(n_paises),
...
def prueba_h5_paises(panel, reporte, carpeta, n_perm=1000, semilla=20260918):
    ...
    p = panel.merge(ia, on=["iso3", "anio"], how="left").sort_values(["iso3", "anio"])
    p["H"] = p[["A", "Dp"]].mean(axis=1)
    g = p.groupby("iso3")
    cons = g["anio"].diff() == 1
